...
class CCTVStream:
    ...
    def generate_frames(self):
        ...
        self.camera.set(cv2.CAP_PROP_POS_FRAMES, 6300)
...
@app.route('/logs/<date>')
def get_logs_by_date_route(date):
    logs_by_date = get_logs_by_date(date)
    return jsonify(logs_by_date)  # JSON 형식으로 반환

@app.route('/index')
def index():
    if 'username' in session:
        date = request.args.get('date', default='2023-06-01')
